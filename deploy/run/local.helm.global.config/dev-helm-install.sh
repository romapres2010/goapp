...
export LOG_FILE=$(echo $LOG_DIR)/$(echo $KUBE_VARIANT)-helm-install.log
...
(cd .. && ./_script/helm-install.sh install $KUBE_VARIANT $KUBE_NAMESPACE $LOG_DIR $HELM_DIR $HELM_VALUES 1>$LOG_FILE 2>&1)
