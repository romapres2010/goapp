...
if [ $# -eq 6 ]
then
    echo "Got 6 arguments: [HELM_ACTION:$1], [KUBE_VARIANT:$2], [KUBE_NAMESPACE:$3], [LOG_DIR:$4], [HELM_DIR:$5], [HELM_VALUES:$5]"
else 
...
export KUBE_DESCRIBE_FILE=$(echo $LOG_DIR)/helm-$(echo $HELM_ACTION)-describe-$(echo $KUBE_VARIANT).log
...
echo "================= helm --dry-run --debug ================="
helm $HELM_ACTION $KUBE_VARIANT $HELM_DIR -f $HELM_VALUES --namespace=$KUBE_NAMESPACE --create-namespace --set appApi.image.repository=$APP_REPOSITORY/$APP_API_APP_NAME --set appApi.image.tag=$APP_VERSION --set version=$APP_VERSION --dry-run --debug 1>$YAML_FILE 2>&1

echo "================= helm ================="
helm $HELM_ACTION $KUBE_VARIANT $HELM_DIR -f $HELM_VALUES --namespace=$KUBE_NAMESPACE --create-namespace --set appApi.image.repository=$APP_REPOSITORY/$APP_API_APP_NAME --set appApi.image.tag=$APP_VERSION  --set version=$APP_VERSION --debug 1>$KUBE_LOG_FILE 2>&1
