...
func IncDBCountVec(sql string) {
	if gMetrics.Cfg.CollectDBCountVec {
		gMetrics.DBCountVec.WithLabelValues(sql).Inc()
	}
}
func AddDBDurationVec(sql string, duration time.Duration) {
	if gMetrics.Cfg.CollectDBDurationVec {
		gMetrics.DBDurationVec.WithLabelValues(sql).Observe(float64(duration.Milliseconds()))
	}
}
...
func IncMarshalingDurationVec(label string, duration time.Duration) {
	if gMetrics.Cfg.CollectMarshalingDurationVec {
		gMetrics.MarshalingDurationVec.WithLabelValues(label).Observe(float64(duration.Milliseconds()))
	}
}
func IncUnMarshalingDurationVec(label string, duration time.Duration) {
	if gMetrics.Cfg.CollectUnMarshalingDurationVec {
		gMetrics.UnMarshalingDurationVec.WithLabelValues(label).Observe(float64(duration.Milliseconds()))
	}
}
